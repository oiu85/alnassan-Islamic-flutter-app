...
class RecentAdvisoriesWidget extends StatelessWidget {
  const RecentAdvisoriesWidget({super.key});
  ...
  Widget _buildContent(BuildContext context, AdvisoryState state) {
    if (state.recentStatus.isLoading()) {
      return Container(
        height: ScreenUtil().setHeight(200),
        child: Center(
          child: Lottie.asset(
        ...
        child: Center(
          child: Column(
            ...
            children: [
              ...
              Text(
                state.recentError ?? 'حدث خطأ في تحميل الفتاوى',
                style: TextStyle(
                  fontFamily: FontFamily.tajawal,
    ...
    if (state.recentAdvisories.isEmpty) {
      return Container(
        height: ScreenUtil().setHeight(200),
        child: Center(
          child: Column(
    ...
    final displayAdvisories = state.recentAdvisories.take(3).toList();
