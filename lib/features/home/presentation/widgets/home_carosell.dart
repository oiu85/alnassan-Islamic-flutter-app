...
class HomeCarousel extends StatefulWidget {
  ...
  const HomeCarousel({super.key, this.importantTopics});
...
class _HomeCarouselState extends State<HomeCarousel> {
  ...
  void _changeText() {
    ...
    try {
      ...
      developer.log('HomeCarousel timer error', name: 'HomeCarousel', error: e);
      _timer = Timer(Duration(seconds: 3), _changeText);
    }
  }
  ...
  void _navigateToAdvisoryViewer(ImportantTopic topic) {
    if (topic.id != null) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => AdvisoryViewerPage(advisoryId: topic.id!),
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final double svgSize = responsiveLayoutValue(
      context,
      mobile: 56.0,
      tablet: 68.0,
    );
